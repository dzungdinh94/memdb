...
global.MONGOOSE_DRIVER_PATH = path.join(__dirname, "mongoose-driver");
  ...
  var promise = P.bind(this)
    ...
    .then(function (ret) {
      ...
      if (opts && opts.backend) {
        return util.promisify(connectMongo, mongoose)(opts.backend.url);
      }
    });
...
mongoose.disconnect = function () {
  var promise = P.bind(this)
    ...
    .then(function () {
      this._autoconn = null;
      return util.promisify(disconnectMongo, mongoose)();
    });
      ...
      for (var i in schema._indexes) {
        ...
        for (field in index[0]) {
          if (index[0][field]) {
            ...
            mdbIndex.valueIgnore[field] =
              paths[field].options.indexIgnore || [];
          }
        }
...
for (let i = overwrited.length - 1; i >= 0; i--) {
  mongoose.Model[overwrited[i]] = function () {
    var cb = arguments[arguments.length - 1];
    if (typeof cb === "function") {
      cb(new Error(util.format("not implemented method: ", overwrited[i])));
    } else {
      throw new Error(util.format("not implemented method: ", overwrited[i]));
    }
  };
}
